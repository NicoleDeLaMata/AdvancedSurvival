...
For such single-event survival data, the **Kaplan-Meier estimator** is the standard for describing time-to-event outcomes. The Kaplan–Meier estimator at time $t_j$ is defined as:
...
where $\lambda_k(t|X_i = x_i)$ is the cause-specific hazard for event type $k$ for a subject with a covariate vector $x_i$, $\lambda_{k,0}(t)$ the baseline cause-specific hazard for event type $k$, and $\beta_k = (\beta_{k1},…,\beta_{kp{})$ ) the $p$-vector of parameters associates with the vector $X$.

Note again, that **individuals experiencing competing events are treated as censored**.

The estimation procedure is identical to the standard Cox PH model for one type of event. Additionally, we can test whether if a covariate’s effect differs across event types by including an interaction term between the covariate and the event type.
